...
export async function GET(req: Request, ctx: { params: { national_id: string } }) {
  try {
    const nationalId = (ctx.params.national_id || '').trim()
    if (!nationalId) return NextResponse.json({ error: 'Missing national id' }, { status: 400 })
    ...
    const owner =
      (data.created_by_email || '').toLowerCase() || (data.applicant_email || '').toLowerCase()
