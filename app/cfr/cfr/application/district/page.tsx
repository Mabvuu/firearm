// app/cfr/cfr/application/district/page.tsx
'use client'
...
export default function CFRDistrictPickPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data, error } = await supabase
        .from('applications')
        .select('id, applicant_name, status, dispol_email')
        .eq('id', appId)
        .single()
  ...
  useEffect(() => {
    const loadPeople = async () => {
      ...
      // ✅ THIS IS THE ROLE YOU SAID: cfr.dispol
      const { data, error } = await supabase
        .from('profiles')
  ...
  const sendToDistrict = async (pick: Pick) => {
    if (!app) return
    setSending(true)
    setErrorMsg(null)
    ...
    // requires columns: dispol_email, cfr_forwarded_by_email, cfr_forwarded_by_uid
    const { error } = await supabase
      .from('applications')
      .update({
        status: 'sent_to_dispol',
        dispol_email: pick.email,
        cfr_forwarded_by_email: user.email,
        cfr_forwarded_by_uid: user.id,
      })
      .eq('id', app.id)
    ...
    <div className="flex min-h-screen">
      ...
      <div className="w-3/4 p-8">
        <Card>
          <CardHeader>
            <CardTitle>
              Send to CFR District Police (cfr.dispol)
          ...
          <CardContent className="space-y-4">
              ...
              <div className="max-h-72 overflow-y-auto">
                ...
                {!filtered.length && (
                  <div className="p-2 text-sm text-muted-foreground">
                    {loadingList ? 'Loading…' : 'No cfr.dispol users found.'}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-between pt-2">
              ...
              <Button variant="outline" onClick={() => router.push('/cfr/cfr/application')}>
                Cancel
              </Button>
            </div>
          </CardContent>
