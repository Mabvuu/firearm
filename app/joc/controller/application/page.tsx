...
export default function JOCControllerApplicationsPage() {
  ...
  // ✅ NEW: decline uses API (server does everything)
  ...
  // ✅ NEW: approve uses API (server does everything)
  const handleAcceptMakeWallet = async (appRow: Application) => {
    try {
      ...
      const res = await fetch('/api/joc/controller/decision', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'approve', applicationId: appRow.id }),
      })

      const data = (await res.json()) as { ok?: boolean; error?: string; walletId?: string }
    ...
    <div className="flex min-h-screen">
      ...
      <div className="w-3/4 p-8">
        <Card>
          ...
          <CardContent>
            {errorMsg && (
              <div className="border rounded p-3 text-sm text-red-600 mb-4">
                {errorMsg}
              </div>
            )}
                    ...
                    <AccordionItem key={app.id} value={String(app.id)}>
                      ...
                      <AccordionContent className="p-4 space-y-4">
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                          ...
                          <Button
                            disabled={isBusy}
                            onClick={() => void handleAcceptMakeWallet(app)}
                          >
